                   ...
                   const char* value)
{
	...
	else if (MATCH("node", "freq1")) {
		pconfig->nodes[node_idx].freq[0] = atof(value);
	}else if (MATCH("node", "freq2")) {
		pconfig->nodes[node_idx].freq[1] = atof(value);
	}else if (MATCH("node", "freq3")) {
		pconfig->nodes[node_idx].freq[2] = atof(value);
	}else if (MATCH("node", "freq4")) {
		pconfig->nodes[node_idx].freq[3] = atof(value);
	}
